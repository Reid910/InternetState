...
- **Embeddings are generated once and clustered in memory — never re-read from the database.** The original clustering design wrote embeddings to Neon during ingest, then re-read them back for every clustering run. On a 15-minute EventBridge cadence this transferred ~3MB of embedding data per run — enough to exhaust Neon's free tier monthly transfer limit in under two weeks. The fix: ingest returns each new article's embedding directly to the caller in memory, and clustering consumes it there without a round-trip to the database. A dedicated `orphan_articles` table handles articles that didn't cluster on their first attempt, storing only the embedding and a foreign key — so unmatched articles get retried on future runs without re-pulling the full historical backlog. Combined, these changes reduced per-run Neon transfer from ~3MB to ~400KB.
